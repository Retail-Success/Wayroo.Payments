...
public class PropayConfigurationParserTests
{
    ...
    private static PropayConfigurationParser CreateParser() => new(new Moq.Mock<ILogger<PropayConfigurationParser>>().Object);
    ...
    /// </summary>
    private static string EventBridgeEnvelope(string accountNumLiteral, bool serializedPayload) =>
        serializedPayload
        ? $@"{{
                ""version"": ""0"",
    ...
    private static string DetailBlockOf(string envelope)
    {
        ...
        return document.RootElement.GetProperty("detail").GetProperty("payload").ToString();
    }

    [Theory]
    [InlineData(true)]
    [InlineData(false)]
    public void Parse_ReadsStringAccountNum_FromRealEnvelope_AndReturnsDetailAsConfiguration(bool useSerializedPayload)
    {
        // The real webhook always quotes accountNum.
        var envelope = EventBridgeEnvelope($"\"{AccountNumber}\"", useSerializedPayload);
    ...
    [InlineData(true)]
    [InlineData(false)]
    public void Parse_ReadsNumericAccountNum_FromTestPayloads(bool useSerializedPayload)
    {
        var envelope = EventBridgeEnvelope(AccountNumber.ToString(), useSerializedPayload);
